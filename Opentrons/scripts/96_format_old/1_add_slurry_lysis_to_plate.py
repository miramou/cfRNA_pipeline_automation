...
#Slurry: Works well if add 10.5 mL slurry to column 1,2 of divided plate holder. Do so when instructed to.
	...
	robot.pause()
	check = input("Add empty plate to %s. Add 10.5 mL slurry to position %s at A2. Add 110 mL lysis buffer (+62 uL ERCC54 Oligo) to position at A1. Seal filled plates while waiting. Press enter to continue. " % (plate_slots[i], i+1, ))
	robot.resume()
